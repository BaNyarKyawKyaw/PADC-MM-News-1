        ...
        loadMMNewsCall.enqueue(new Callback<GetNewsResponse>() {
            ...
            public void onResponse(Call<GetNewsResponse> call, Response<GetNewsResponse> response) {
                ...
                if (getNewsResponse != null && getNewsResponse.getNewsList().size() > 0) {
                    RestApiEvents.NewsDataLoadedEvent newsDataLoadedEvent = new RestApiEvents.NewsDataLoadedEvent(getNewsResponse.getPageNo()
                            , getNewsResponse.getNewsList());
                    EventBus.getDefault().post(newsDataLoadedEvent);
                }
                else{
                    RestApiEvents.EmptyResponseEvent errorEvent= new RestApiEvents.EmptyResponseEvent("No Data could be loaded for now. Pls try again later.");
